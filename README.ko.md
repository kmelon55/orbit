...
서버에 지정한 초기 비밀번호로 로그인한 뒤 **설정 → 계정**에서 로그인할 때 사용할 계정 이름과 비밀번호를 변경할 수 있습니다. 현재 비밀번호로 확인한 뒤 저장하며, 새 비밀번호는 변경할 때만 12자 이상 입력하세요. 비워두면 현재 비밀번호를 유지합니다. 현재 기기의 로그인은 유지되고 다른 기기의 세션은 즉시 무효화됩니다. 계정 변경은 로그인한 사용자만 할 수 있습니다.

변경한 비밀번호는 scrypt 해시와 별도 세션 키로 `<ORBIT_VAULT_DIR>/.orbit/auth.json`에 저장됩니다(파일 권한 0600). 영구 볼륨을 유지하면 재배포 후에도 적용되며, 저장된 계정 이름과 비밀번호가 초기 `ORBIT_AUTH_USERNAME`·`ORBIT_AUTH_PASSWORD`보다 우선합니다. 저장 후에는 두 환경변수를 제거해도 기존 계정을 사용할 수 있습니다. 이 파일은 노트 내보내기에 포함되지 않으므로 서버 볼륨 백업에 별도로 보존하세요. 파일이 손상되면 초기 비밀번호로 되돌아가지 않고 로그인을 차단합니다.

비밀번호를 잊었다면 서버 관리자가 서비스를 중지하고 `auth.json`을 안전한 위치로 옮긴 뒤 `ORBIT_AUTH_USERNAME`과 새 `ORBIT_AUTH_PASSWORD`를 설정하고 재시작해야 합니다. 이후 로그인하고 설정에서 비밀번호를 다시 변경하세요. 일반 비밀번호 변경에는 서버 재시작이 필요하지 않습니다.
