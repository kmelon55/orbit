...
로그인 화면의 **비밀번호 변경**에서 아이디·현재 비밀번호·새 비밀번호(12자 이상)를 입력할 수 있습니다. 변경 후에는 새 비밀번호로 다시 로그인하세요. 모든 기존 세션은 즉시 무효화됩니다.
...
비밀번호를 잊었다면 서버 관리자가 서비스를 중지하고 `auth.json`을 안전한 위치로 옮긴 뒤 새 `ORBIT_AUTH_PASSWORD`를 설정하고 재시작해야 합니다. 이후 로그인 화면에서 비밀번호를 다시 변경하세요. 일반 비밀번호 변경에는 서버 재시작이 필요하지 않습니다.
