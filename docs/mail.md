...
- 받은 메일·보낸 메일·휴지통·보관함, 검색, 계정별 이전 메일 페이지를 지원합니다. 통합 목록은 계정별 최근 50개부터 표시하고, 이전 메일도 모든 계정에서 함께 불러올 수 있습니다. 저장된 목록을 먼저 보여주고 계정별 최신화 상태와 실패를 표시합니다. 검색은 현재 메일함에서 제목·보낸 사람·받는 사람·참조·본문을 제공자 서버에 요청하며, 저장된 제목·주소·미리보기 결과를 먼저 표시합니다. 한글 조합 중에는 검색 요청을 보내지 않습니다.
- 제공자 대화 ID와 실제 답장 참조값으로 메일을 자동으로 묶습니다. 받은 메일은 왼쪽, 보낸 메일은 오른쪽 말풍선으로 구분해 본문을 오래된 순서로 펼쳐 보여줍니다. 각 메일에서 바로 답장·전체 답장·전달할 수 있으며, 긴 대화의 본문도 스크롤하면 자동으로 불러옵니다. Gmail은 제공자의 대화 전체를 불러오고, iCloud·네이버의 이전 대화는 받은/보낸 메일함에서 제목으로 후보를 검색한 뒤 참조값으로 검증합니다. 후보 페이지를 넘어가는 긴 대화나 조회 실패는 일부만 표시됐음을 안내합니다.
...
### iCloud 사용자 설정 도메인·추가 주소

같은 Apple 계정에 연결된 도메인은 기본 `@icloud.com` 주소와 앱 전용 비밀번호로 한 번만 연결합니다. **메일 설정 → 해당 iCloud 계정 → 도메인·발신 주소**에 Apple에서 이미 사용 중인 전체 메일 주소를 한 줄에 하나씩 등록하고 기본 발신 주소를 선택하세요. Orbit은 도메인이나 주소를 Apple에 새로 생성하지 않습니다.

- 상단 계정 선택에 각 주소가 개별 메일함처럼 표시됩니다. 하나 또는 여러 주소를 선택해 받은·보낸·보관·휴지통 메일을 확인하며, 모든 계정에서는 중복 없이 모아 봅니다. 실제 연결·비밀번호·알림 설정은 iCloud 계정 하나를 공유합니다.
- 주소별 검색은 iCloud 서버의 과거 메일까지 조회합니다. 받은 메일은 To/Cc 및 제공된 Delivered-To/X-Original-To, 보낸 메일은 From 주소로 정확히 구분합니다. 숨은 참조·메일링 리스트처럼 원래 수신 주소가 헤더에 없으면 분류를 추측하지 않으며, **모든 계정**에서 확인할 수 있습니다.
- 새 메일은 현재 선택한 주소를 우선 사용하고, 통합 보기에서는 기본 발신 주소를 사용합니다. 작성창의 **보내는 사람**에서 주소를 변경할 수 있습니다. 답장은 받은 주소를 자동 선택하며 같은 iCloud 계정의 다른 등록 주소로 변경할 수 있습니다. 전체 답장에서 연결된 본인 주소들은 제외합니다.
- 주소별 새 메일 초안을 분리하고 발신 주소를 함께 임시저장합니다. 주소를 삭제해도 원본 메일이나 초안은 삭제되지 않으며, 삭제한 주소가 선택된 초안은 등록된 주소로 변경해야 발송됩니다.
- 발신 주소는 서버에서도 등록 목록을 확인합니다. SMTP 로그인은 기본 iCloud 계정으로 유지하고,메일의 From과 SMTP envelope에는 선택한 주소를 사용합니다. 실제 발송 가능 여부는 Apple에 해당 주소가 정상 설정되어 있어야 하며, 주소 등록만으로 SMTP 발송 검증을 했다는 뜻은 아닙니다.

한 연결에 여러 주소를 두고 답장 수신 주소를 자동 선택하는 방식은 [Thunderbird의 identities](https://support.mozilla.org/en-US/kb/using-identities)와 [Apple Mail의 사용자 설정 도메인](https://support.apple.com/en-am/guide/mail/mlhl1b36a5a9/mac)을 참고했습니다.
...
- `mail.sqlite`: 계정 메타데이터, 암호화된 인증정보·메일 목록 캐시·초안·푸시 구독, 발송 요청 기록, 알림 큐. 정리된 메일 본문은 외부 이미지 차단/허용 버전을 암호화해 최대 100개·7일 동안 캐시합니다(메일당 암호화 데이터 4MiB 이하). 원본과 수신 첨부파일은 요청 시 제공자에서 가져오며 디스크에 캐시하지 않습니다. iCloud·네이버는 목록과 본문용 연결을 분리해 재사용하고 유휴 60초 후 종료합니다.
...
- 브라우저 서비스워커는 정적 아이콘만 캐시하며 개인 페이지·메일 API·첨부파일을 캐시하지 않습니다. 메일 HTML은 서버에서 정리한 뒤 같은 출처 권한이 없는 iframe에 표시합니다. 발신자의 스크립트·이벤트·폼은 제거하고 nonce로 허용된 본문 높이 조정 코드만 실행합니다. 인라인 이미지와 HTTPS 외부 이미지를 자동으로 표시하며 외부 이미지는 본문에서 숨길 수 있습니다.
...
`/mail?demo=1`에서 개인·업무 iCloud 예시 계정 두 개와 뉴스레터, 인라인/외부 이미지, 주문 내역 표, 첨부파일, 주고받은 대화를 확인할 수 있습니다. 실제 메일과 같은 MIME 파서·HTML 정리·화면 컴포넌트를 사용합니다. 검색·계정 전환·읽음 처리·작성·답장을 지원하지만 발송은 데모 보낸 메일함에만 저장되고 외부로 전송되지 않습니다. 실제 계정·초안·메일 DB와 분리되어 있으며, 서버 메모리에 보관하므로 새로고침에는 유지되고 서버 재시작 또는 **초기화**로 복원됩니다.

데모의 개인 계정에는 `hello@mina.design`, `work@mina.studio` 주소가 함께 있습니다. 메일 설정에서 데모 주소와 기본 발신 주소를 변경하고, 주소별 조회·답장·발신 주소 선택을 확인할 수 있습니다. 데모 주소 변경도 실제 계정 설정과 분리되어 있습니다.
